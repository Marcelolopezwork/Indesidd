		...
		element
			...
			.on('click', function () {
				if ($(this).next().is(':hidden')) {
					...
					$(this)
						.toggleClass('accordion-active ' + elActiveClass)
						.next()
						.stop(true, true)
...
window.SEMICOLON_portfolioLoadItem = function (
	portPostId,
	prevPostPortId,
	getIt
) {
...
window.SEMICOLON_carouselInit = function ($carouselEl) {
	...
	$carouselEl.each(function () {
		...
		$(window).on('lazyLoadLoaded', function () {
			if (
				element.find('.lazy').length == element.find('.lazy.lazy-loaded').length
			) {
...
window.SEMICOLON_flexSliderInit = function ($flexSliderEl) {
	...
	$flexSliderEl.each(function () {
		let element = $(this),
			...
			elArrowLeft = element.attr('data-arrow-left') || 'icon-line-chevron-left',
			elArrowRight =
				element.attr('data-arrow-right') || 'icon-line-chevron-right',
			...
			function () {
				...
				t = setTimeout(function () {
					element
						...
						.removeClass(elAnimateOut + ' not-animated')
						.addClass(elAnimate + ' animated');
				}, elDelayT);
			},
			function () {
				element
					.addClass('not-animated')
					.removeClass(elAnimate + ' not-animated')
					.addClass(elAnimateOut + ' animated');
				if (elReset == 'true') {
					x = setTimeout(function () {
						element
							.removeClass(elAnimateOut + ' animated')
							.addClass('not-animated');
					}, Number(elSpeed));
				}
				clearTimeout(t);
...
window.SEMICOLON_getInstagramPhotos = function (
	element,
	username,
	limit,
	nextkey,
	...
	let newimages = '';
	$.getJSON(
		'https://www.instagram.com/' + username + '/?__a=1' + nextkey,
		function (instagram) {
			...
			if (!instaPrivate) {
				...
				if (images.length < limit) {
					images = SEMICOLON_getInstagramPhotos(
						element,
						username,
						limit,
						nextkey,
				...
				if (images.length > 0) {
					...
					for (let i = 0; i < limit; i++) {
						...
						html =
							html +
							'<a class="grid-item" href="https://instagram.com/p/' +
							photo.node.shortcode +
							'" target="_blank"><img src="' +
							photo.node.thumbnail_resources[2].src +
							'" alt="Image"></a>';
					}
...
window.SEMICOLON_gridContainerInit = function ($gridContainerEl) {
	...
	$gridContainerEl.each(function () {
		let element = $(this),
			...
			elBase =
				element.attr('data-basewidth') || '.portfolio-item:not(.wide):eq(0)',
			elOriginLeft = true,
			elGrid;
		...
		let elementInterval = setInterval(function () {
			if (
				element.find('.lazy.lazy-loaded').length == element.find('.lazy').length
			) {
				setTimeout(function () {
				...
				function () {
					if (elHighlight) {
						if ($(elAnchor).find('.highlight-me').length > 0) {
							$(elAnchor)
								.find('.highlight-me')
								.animate({ backgroundColor: elHighlight }, 300);
							setTimeout(function () {
								$(elAnchor)
									.find('.highlight-me')
...
window.SEMICOLON_readmoreInit = function ($readmoreEl) {
	...
	$readmoreEl.each(function () {
		let element = $(this),
			...
			elSize = element.attr('data-readmore-size') || '5rem',
			...
			on: {
				...
				transitionEnd: function (swiper) {
					element.find('.swiper-slide').each(function () {
						...
						if (
							slideEl.find('.yt-bg-player.mb_YTPlayer:not(.customjs)').length >
							0
						) {
							slideEl
